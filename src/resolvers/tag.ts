...
export class TagResolver {
	...
	@UseMiddleware(isAuthenticated)
	@Query(() => Tag)
	async getTagById(@Arg('id', () => ID) id: string, @Ctx() { req }: Context): Promise<Tag> {
		const tag = await postgresdb.getRepository(Tag).findOne({ where: { id, userId: req.user?.id } })
	...
	@UseMiddleware(isAuthenticated)
	@Mutation(() => Tag)
	async createTag(@Arg('input') tagInput: TagInput, @Ctx() { req }: Context): Promise<Tag> {
	...
	@UseMiddleware(isAuthenticated)
	@Mutation(() => Tag)
	async updateTag(@Arg('input') tagInput: TagInput, @Ctx() { req }: Context): Promise<Tag> {
	...
	@UseMiddleware(isAuthenticated)
	@Mutation(() => Boolean)
	async deleteTag(@Arg('id', () => ID) id: string, @Ctx() { req }: Context): Promise<boolean> {
		const tag = await postgresdb.getRepository(Tag).findOne({
